...
public class TokenUtil {
    ...
    private static final String SECRET_KEY = "K7#7zJ=>f>>skPC=Ay3I7leR*NKP!$#d";
    ...
    public static Authentication validate(HttpServletRequest request) {
        ...
        if (isSubjectValid(username) && isEmissorValid(issuer) && isExpirationValid(expira)) {
            return new UsernamePasswordAuthenticationToken(username, null, Collections.emptyList()); //1. username 2. perfil (adm ou algo assim) 3. lista de endpoints que pode acessar
        }
        return null;
